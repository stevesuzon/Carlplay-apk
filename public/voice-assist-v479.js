(function(){
'use strict';
if(window.__carplayVoiceV479Loaded)return;
window.__carplayVoiceV479Loaded=true;

var PRESS_MS=1200;
var SESSION_KEY='carplay_voice_prompt_confirmed_v479';
...
function toast(msg){
  var el=document.getElementById('carplayVoiceToastV479');
  if(!el){el=document.createElement('div');el.id='carplayVoiceToastV479';el.style.cssText='position:fixed;left:50%;bottom:22px;transform:translateX(-50%);z-index:2147483646;max-width:88vw;padding:11px 15px;border-radius:14px;background:#07111df2;border:2px solid #f39b19;color:#fff;font:900 14px Arial,sans-serif;text-align:center;box-shadow:0 8px 24px #0009;pointer-events:none';document.body.appendChild(el)}
...
function showFirstPrompt(text){
  var old=document.getElementById('voicePromptV479');if(old)old.remove();
  var b=document.createElement('button');b.id='voicePromptV479';b.type='button';
...
function targetFromEvent(e){
  ...
  if(el.closest('#voicePromptV479'))return null;
...
document.addEventListener('touchmove',moveTouchV479,{capture:true,passive:true});
document.addEventListener('touchend',finishTouchV479,{capture:true,passive:false});
document.addEventListener('click',function(e){
  if(e.target&&e.target.closest&&e.target.closest('#voicePromptV479'))return;
...
(function installGlobalVoiceStyleV478(){
  if(document.getElementById('carplayVoiceGlobalStyleV479'))return;
  var st=document.createElement('style');st.id='carplayVoiceGlobalStyleV479';
