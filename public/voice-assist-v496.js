(function(){
  ...
  function installNoSelectV389(){
    ...
    st.textContent='[data-voice-card],[data-voice-card] *,button,a,[onclick],[role="button"]{-webkit-user-select:none!important;user-select:none!important;-webkit-touch-callout:none!important}[data-voice-card]{touch-action:pan-y pinch-zoom}';
  ...
  var pageVoiceUnlockedV496=false;
  var pageVoiceActivatingV496=false;
  function pageVoiceReadyV496(){return pageVoiceUnlockedV496===true}
  function activatePageVoiceV496(){
    if(pageVoiceUnlockedV496||pageVoiceActivatingV496)return true;
    if(!('speechSynthesis' in window)||typeof SpeechSynthesisUtterance==='undefined')return false;
    pageVoiceActivatingV496=true;
    try{
      window.speechSynthesis.cancel();
      var u=new SpeechSynthesisUtterance('Son activé');
      u.lang='fr-FR';u.rate=.95;u.pitch=1;u.volume=1;
      u.onstart=function(){
        pageVoiceUnlockedV496=true;
        pageVoiceActivatingV496=false;
        unlockVoiceV456();
      ...
      u.onerror=function(){
        pageVoiceActivatingV496=false;
        pageVoiceUnlockedV496=false;
        toast('🔇 Touchez encore une fois pour activer le son de cette page.');
      };
  ...
  function spokenText(s){return clean(String(s||'').replace(/[\p{Extended_Pictographic}\uFE0F\u200D]/gu,' ').replace(/[←→›⌄⚙★☆]/g,' '))}
  ...
  function pickV486(a){return a[Math.floor(Math.random()*a.length)]||''}
  function isOiseV486(card){
  ...
  function marketHumorV486(card){
    ...
    if(isOiseV486(card)){
      ...
      if(city.indexOf('senlis')>=0||name.indexOf('senlis')>=0){
        lines.push("Attention, vous êtes dans le quartier à Isaac. C'est un garçon qui n'est pas du genre à donner un bâton pour se faire battre. Mais si vous voulez y aller, alors");
      }
      return pickV486(lines);
    }
    return pickV486([
  ...
  function buttonSpeech(el){
    ...
    if(el.tagName==='SELECT'){
      ...
      var txt=spokenText(opt&&opt.textContent||'');
      return txt?txt+'.':'';
    }
    var type=String(el.type||'').toLowerCase();
    if(type==='checkbox'||type==='radio'){
      var lab=el.closest&&el.closest('label');
      if(!lab&&el.id){try{lab=document.querySelector('label[for="'+String(el.id).replace(/"/g,'\\\"')+'"]')}catch(_){}}
      var lt=spokenText(lab&&(lab.innerText||lab.textContent)||'');
      return lt?lt+'.':'';
    }
    var txt=spokenText(el.innerText||el.textContent||'');
    if(!txt&&el.value)txt=spokenText(el.value);
    if(!txt)return '';
  ...
  function simpleSpeakV495(text){
    text=spokenText(text);
    if(!text||!enabled())return false;
    if(!pageVoiceReadyV496()){
      toast('🔊 Touchez une fois pour activer le son de cette page.');
      return false;
    }
    if(!('speechSynthesis' in window)||typeof SpeechSynthesisUtterance==='undefined')return false;
    try{
  ...
  function speak(text){
    ...
    return simpleSpeakV495(text);
  }
  function eligibleTarget(e){
    if(!e||!e.target||!e.target.closest)return null;
    if(e.target.closest('#voiceTapFallbackV400'))return null;
    var card=e.target.closest('[data-voice-card="market"]');
    var control=e.target.closest('button,a,select,input[type="button"],input[type="submit"],input[type="checkbox"],input[type="radio"],summary,[onclick],[role="button"],[role="menuitem"],[role="option"],[data-action],[data-target],.go,.open,.verify,.details,.register,.day,.dayBtn,.tab,.tile,.directBtn,.homeTopButton,.settingHead,.changeArea,.back');
    if(control&&(!card||control!==card))return control;
    if(card)return card;
    return e.target.closest('[data-voice-read],[data-voice-help],#status,.status,.sectionTitle,.heading,.marketPeriodMain,.marketPeriodCount,.marketPeriodExtra,h1,h2,h3,.specialBadge,.specialDate,.specialTimer,.tradeStatus,.card,.small,.market,.market-card,.marketCard,.station-card,.stationCard,.result-card,.resultCard');
  }
  ...
  function begin(e){
    ...
    if(!pageVoiceReadyV496()){
      activatePageVoiceV496();
      suppressClickTarget=card;suppressClickUntil=Date.now()+1800;
      return;
    }
  ...
  function showTapFallbackV400(text){
    text=spokenText(text);if(!text)return false;
    if(voiceUnlockedV456())return simpleSpeakV495(text);
    var old=document.getElementById('voiceTapFallbackV400');if(old)old.remove();
    var b=document.createElement('button');
    b.id='voiceTapFallbackV400';b.type='button';
    b.setAttribute('aria-label','Activer le son pour '+text);
    b.innerHTML='<span style="display:block;font-size:42px;line-height:1;margin-bottom:12px">🔊</span><span style="display:block;font-size:21px;line-height:1.15">ACTIVER LE SON</span><span style="display:block;margin-top:12px;font-size:13px;line-height:1.25;color:#d8ecff">Touchez une fois. Le son restera actif jusqu’à la fermeture de l’application.</span>';
  ...
  function speakDirectFromGestureV491(text){return simpleSpeakV495(text)}
  function speakImmediateV400(text){return simpleSpeakV495(text)}
  ...
  function touchBeginV394(e){
    ...
    // V496 : chaque page demande son propre premier geste iPhone.
    // Le premier appui est réservé à l'activation vocale de cette page.
    if(!pageVoiceReadyV496()){
      active=null;touchTargetV398=null;fired=false;touchReadyV395=false;touchActionDoneV456=false;
      activatePageVoiceV496();
      suppressClickTarget=target;suppressClickUntil=Date.now()+1800;
      return;
    }
    ...
    timer=setTimeout(function(){
      ...
      queuedTouchTextV400='';
      if(voiceUnlockedV456())simpleSpeakV495(text);else showTapFallbackV400(text);
    },PRESS_MS);
  }
  ...
  document.addEventListener('click',function(e){
    ...
    if(!pageVoiceReadyV496()&&e.target&&e.target.closest){
      var first=eligibleTarget(e);
      if(first){
  ...
  window.CouteauVoice={enabled:enabled,setEnabled:setEnabled,speak:speak,buildMarket:buildMarket,pressMs:PRESS_MS,version:'V496'};
})();
