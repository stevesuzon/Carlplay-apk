(function () {
  ...
  function renderMarkets() {
    ...
    for (i = 0; i < rs.length; i++) {
      ...
      html +=
        '<article class="card" data-voice-card="market" data-voice-department="'+esc(r[0]||'')+'" data-voice-city="'+esc(place)+'" data-voice-name="'+esc(name)+'" data-voice-day="'+esc(voiceDayLabel(currentDay))+'" data-voice-distance="'+esc(marketDistanceText(r)||'')+'" data-voice-count="'+esc(count)+'" data-voice-count-label="commerçants">' + verificationDot(r, s) + '<button type="button" class="marketFavoriteStar ' +
        (fav ? "active" : "") +
        '" aria-label="' +
