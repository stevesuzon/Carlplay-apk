(function(){
  'use strict';
  if(window.__carplayVoiceV462Loaded)return;
  window.__carplayVoiceV462Loaded=true;
  var KEY='carplay_voice_enabled_v387';
  var PRESS_MS=1200;
  var active=null,timer=0,startX=0,startY=0,fired=false,suppressClickTarget=null,suppressClickUntil=0,preferredVoice=null,touchStartedAtV395=0,touchReadyV395=false,currentUtteranceV396=null,primeUtteranceV396=null,touchTargetV398=null,suppressAllClicksUntilV398=0,queuedTouchTextV400='',voiceAwakeV423=false,touchActionDoneV456=false,gesturePrimedV462=false;
  ...
  function setEnabled(on){
    on=!!on;voiceAwakeV423=false;
    try{localStorage.setItem(KEY,on?'1':'0')}catch(_){}
    try{document.cookie='carplay_voice_enabled='+(on?'1':'0')+'; path=/; max-age=31536000; SameSite=Lax'}catch(_){}
    if(on){unlockVoiceV456();}
    else{
      try{sessionStorage.removeItem('carplay_voice_unlocked_v456')}catch(_){}
      try{if(window.speechSynthesis)window.speechSynthesis.cancel()}catch(_){}
      var f=document.getElementById('voiceTapFallbackV400');if(f)f.remove();
    }
    syncSetting();
    try{window.dispatchEvent(new CustomEvent('carplay-voice-change',{detail:{enabled:on}}))}catch(_){}
  }
  function clean(s){return String(s||'').replace(/\s+/g,' ').trim()}
  function removeOldVoiceFallbackV460(){var f=document.getElementById('voiceTapFallbackV400');if(f)f.remove()}
  function speechCleanV459(value){
    var t=String(value||'');
    try{t=t.replace(/[\u{1F000}-\u{1FAFF}]/gu,' ')}catch(_){}
    t=t.replace(/[\u2190-\u21FF\u2300-\u27BF\uFE0F\u200D]/g,' ');
    return clean(t);
  }
  ...
  function marketAreaV462(card){
    var area=clean(card&&card.dataset&&(card.dataset.voiceArea||card.dataset.department)||'');
    if(area)return area.toLowerCase();
    var key=clean(card&&card.dataset&&card.dataset.marketKey||'');
    if(key){var bits=key.split('|');if(bits.length>1)return clean(bits[1]).toLowerCase()}
    try{var q=new URLSearchParams(location.search);var a=clean(q.get('area')||'');if(a)return a.toLowerCase()}catch(_){}
    return '';
  }
  function marketJokeV462(card){
    if(marketAreaV462(card)!=='60')return '';
    var jonathan=[
      "Attention, vous êtes sur le marché à Jonathan, le vaqueso du 60. S'il est déjà passé, ça va être compliqué de trouver des clients.",
      "Jonathan est peut-être déjà passé sur celui-là. Si c'est le cas, ça sert à rien de courir, il a déjà pris tous les clients.",
      "Attention, marché à Jonathan, le vaqueso du 60. S'il arrive avant vous, il ne va pas laisser beaucoup de clients derrière lui."
    ];
    var isaac=[
      "Attention, vous êtes dans le quartier à Isaac, le vaqueso du 60. C'est un garçon qui n'est pas du genre à donner un bâton pour se faire battre.",
      "Ça sert à rien de revenir sur ce marché. Isaac est déjà venu la semaine dernière, il a déjà pris tous les clients.",
      "Isaac, le vaqueso du 60, connaît déjà ce marché. S'il est passé avant vous, il a déjà fait les clients."
    ];
    var both=[
      "Attention, ce marché pourrait avoir Jonathan et Isaac, les deux vaqueso du 60. S'ils sont là avant vous, bon courage pour trouver des clients.",
      "Jonathan et Isaac sur le même marché, les deux vaqueso du 60. Là, il faut arriver de bonne heure si vous voulez encore voir un client.",
      "Attention, secteur Jonathan et Isaac, les deux vaqueso du 60. Si les deux sont déjà passés, ça sert à rien de revenir, ils ont pris tous les clients."
    ];
    var groups=[jonathan,isaac,both],g=groups[Math.floor(Math.random()*groups.length)];
  ...
  function buildMarket(card){
    ...
    parts.push(marketJokeV462(card));
    var day=clean(card.dataset.voiceDay||'');
    if(day)parts.push(day);
    var distance=clean(card.dataset.voiceDistance||'');
    if(distance)parts.push('distance '+distance.replace(/\bkm\b/gi,'kilomètres'));
  ...
  function buttonSpeech(el){
    ...
    var txt='';
    if(el.getAttribute)txt=speechCleanV459(el.getAttribute('data-voice-help')||'');
    if(!txt)txt=speechCleanV459(el.innerText||el.textContent||'');
    if(!txt&&el.getAttribute)txt=speechCleanV459(el.getAttribute('aria-label')||el.getAttribute('title')||'');
    ...
    txt=speechCleanV459(txt).replace(/[•·]/g,', ').replace(/\bKM\b/gi,'kilomètres').replace(/\s+/g,' ').trim();
  ...
  function buildSpeech(card){
    ...
    if(card.dataset.voiceText)return speechCleanV459(card.dataset.voiceText);
    var kind=card.dataset.voiceCard||'market';
    if(kind==='market')return buildMarket(card);
    return speechCleanV459(card.innerText||card.textContent||'');
  }
  function toast(msg){
  ...
  function primeFromGestureV462(){
    if(!enabled()||gesturePrimedV462||!('speechSynthesis' in window)||typeof SpeechSynthesisUtterance==='undefined')return;
    try{
      ...
      u.lang='fr-FR';u.volume=0.01;u.rate=10;u.pitch=1;if(preferredVoice)u.voice=preferredVoice;
      u.onstart=function(){gesturePrimedV462=true;voiceAwakeV423=true};
      u.onend=function(){primeUtteranceV396=null;gesturePrimedV462=true;voiceAwakeV423=true};
      u.onerror=function(){primeUtteranceV396=null;gesturePrimedV462=false;voiceAwakeV423=false};
      window.speechSynthesis.speak(u);gesturePrimedV462=true;
    }catch(_){gesturePrimedV462=false}
  }
  ...
  function resetSpeechV423(){
    voiceAwakeV423=false;gesturePrimedV462=false;
    preferredVoice=null;
  ...
  function eligibleTarget(e){
    if(!enabled()||!e||!e.target||!e.target.closest)return null;
  ...
  function showTapFallbackV400(text){
    text=speechCleanV459(text);if(!enabled()||!text)return false;
    toast('🔇 Refaites un appui long sur le bouton.');
    return false;
  }
  function speakImmediateV400(text){
    text=speechCleanV459(clean(text));if(!text||!enabled())return false;
    if(!('speechSynthesis' in window)||typeof SpeechSynthesisUtterance==='undefined'){toast('🔇 Lecture vocale indisponible sur cet appareil.');return false}
    unlockVoiceV456();
    try{
      ...
      u.onend=function(){currentUtteranceV396=null;voiceAwakeV423=true};
      u.onerror=function(){currentUtteranceV396=null;toast('🔇 La voix n’a pas démarré. Relâchez puis refaites un appui long.')};
      window.speechSynthesis.speak(u);
      return true;
    }catch(_){currentUtteranceV396=null;toast('🔇 Lecture vocale indisponible.');return false}
  }
  ...
  function syncSetting(){
    var on=enabled(),t=document.getElementById('voiceAssistToggle'),st=document.getElementById('voiceAssistStatus');
    document.documentElement.classList.toggle('carplayVoiceEnabled',on);
    if(t)t.checked=on;
    if(st)st.textContent=on?'🔊 Voix activée — appui long 1,20 seconde sur une fiche ou un bouton.':'🔇 Voix désactivée.';
  }
  function initSetting(){
    removeOldVoiceFallbackV460();loadPreferredVoice();syncSetting();
    var t=document.getElementById('voiceAssistToggle');
    if(t&&!t.dataset.voiceWired){t.dataset.voiceWired='1';t.addEventListener('change',function(){setEnabled(t.checked)})}
  }
  window.voiceSettingsDirect=function(on){
    setEnabled(!!on);
    if(on){unlockVoiceV456();speakImmediateV400('Lecture vocale activée.')}
  };
  installNoSelectV389();
  ...
  window.addEventListener('pageshow',function(){removeOldVoiceFallbackV460();resetSpeechV423()});
  ...
  window.CouteauVoice={enabled:enabled,setEnabled:setEnabled,speak:speak,buildMarket:buildMarket,prime:primeFromGestureV462,pressMs:PRESS_MS};
})();
