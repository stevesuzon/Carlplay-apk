(function(){
'use strict';
if(window.__couteauVoiceV484Loaded)return;
window.__couteauVoiceV484Loaded=true;

var PRESS_MS=1200;
var SESSION_KEY='couteau_voice_active_v484';
var press=null;
var timer=0;
var suppressUntil=0;
...
function activationSquare(text){
  ...
  var old=document.getElementById('couteauVoiceActivateV484');
  if(old)old.remove();

  var box=document.createElement('button');
  box.id='couteauVoiceActivateV484';
  box.type='button';
...
function targetFromPoint(el){
  ...
  if(el.closest('#couteauVoiceActivateV484'))return null;
...
function triggerLongPress(){
  ...
  var text=buildSpeech(press.target);
  if(!text){clearPress();return}
  press.done=true;
  suppress(press.target);
...
function startTouch(e){
  ...
  if(!target)return;
  var t=e.touches[0];
  press={target:target,type:'touch',x:Number(t.clientX||0),y:Number(t.clientY||0),started:Date.now(),done:false};
  timer=setTimeout(triggerLongPress,PRESS_MS);
}
function moveTouch(e){
  if(!press||press.type!=='touch'||press.done||!e.touches||e.touches.length!==1)return;
  var t=e.touches[0];
  if(Math.abs(Number(t.clientX||0)-press.x)>32||Math.abs(Number(t.clientY||0)-press.y)>32)clearPress();
}
function endTouch(e){
  ...
  if(!p.done&&Date.now()-p.started>=PRESS_MS){
    press=p;
    triggerLongPress();
  }
  var wasLong=!!(press&&press.done)||p.done;
  press=null;
  if(wasLong)stopEvent(e);
}
...
function startPointer(e){
  ...
  var target=targetFromPoint(e.target);
  if(!target)return;
  press={target:target,type:'pointer',id:e.pointerId,x:Number(e.clientX||0),y:Number(e.clientY||0),started:Date.now(),done:false};
  timer=setTimeout(triggerLongPress,PRESS_MS);
}
function movePointer(e){
  if(!press||press.type!=='pointer'||press.id!==e.pointerId||press.done)return;
  if(Math.abs(Number(e.clientX||0)-press.x)>32||Math.abs(Number(e.clientY||0)-press.y)>32)clearPress();
}
...
document.addEventListener('click',function(e){
  if(e.target&&e.target.closest&&e.target.closest('#couteauVoiceActivateV484'))return;
...
(function style(){
  ...
  st.id='couteauVoiceStyleV484';
  st.textContent='button,a,select,summary,[role="button"],[role="menuitem"],[role="option"],[data-voice-card],#status,.status,.sectionTitle,.heading,h1,h2,h3{-webkit-user-select:none!important;user-select:none!important;-webkit-touch-callout:none!important}[data-voice-card]{touch-action:pan-y pinch-zoom}';
