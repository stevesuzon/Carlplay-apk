(function sharedMarketConsensus(){
  ...
  function photoCacheKey(card){return 'marketPhotoFastV492:'+(card&&card.dataset.marketKey||'')}
  function cachedPhoto(card){
    if(!card||!card.dataset.marketKey)return null;
    try{
      var value=JSON.parse(localStorage.getItem(photoCacheKey(card))||'null');
  ...
  function updatePhotoSlot(card,state){
    var add=card.querySelector('.addMarketPhoto'),img=card.querySelector('.sharedMarketPhoto'),firstMeta=card.querySelector('.meta'),
        photo=(state&&state.photo)||cachedPhoto(card);
    if(photo){
      rememberPhoto(card,photo);
      if(add)add.remove();
      if(!img){
