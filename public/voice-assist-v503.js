(function(){
'use strict';
if(window.__voiceClean503)return;window.__voiceClean503=1;
...
function frenchVoice(){
 try{
   ...
   for(var i=0;i<vs.length;i++)if(/^fr[-_]/i.test(vs[i].lang||''))return vs[i];
 }catch(_){}
 return null;
}
function say(t){
 ...
 try{
   // Reprise du moteur qui arrivait réellement à activer la voix sur iPhone.
...
function arm(){return armFromGesture()}
function target(n){
...
function fireLong(el){
 ...
 // Comme dans l'ancienne version qui activait correctement le son :
 // à 1,20 s on valide l'appui long, mais on parle au touchend,
 // qui est un vrai geste utilisateur reconnu par iOS.
}
function start(e){
...
function installLongPressProtection(){
 try{
   ...
   st.id='voiceLongPressProtectionV502';
   st.textContent=
    'html,body{-webkit-touch-callout:none!important;}'+
    'button,a,[role="button"],[onclick],[data-action],[data-target],[data-voice-card],.card,[class*="card"],.fiche,[class*="fiche"]{-webkit-touch-callout:none!important;-webkit-user-select:none!important;user-select:none!important;}'+
    'img{pointer-events:none!important;-webkit-touch-callout:none!important;-webkit-user-select:none!important;user-select:none!important;-webkit-user-drag:none!important;}';
 ...
 function lockImages(root){
   try{
     ...
     for(var i=0;i<list.length;i++){
       ...
       list[i].style.webkitUserDrag='none';
       list[i].style.webkitTouchCallout='none';
     ...
     for(var i=0;i<ms.length;i++){
       ...
       for(var j=0;j<ns.length;j++){
         ...
         if(n.tagName==='IMG'){
           ...
           n.style.webkitUserDrag='none';n.style.webkitTouchCallout='none';n.style.pointerEvents='none';
         }
         lockImages(n);
       }
     }
...
window.CouteauVoice={enabled:on,setEnabled:setOn,speak:function(t){return armed?say(t):false},pageArmed:function(){return armed},activatePage:arm,buildMarket:market,version:'V503'};
})();