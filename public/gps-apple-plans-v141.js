(function(){
  ...
  function preferredPlaceTitle(name,fullAddress){
    var n=cleanPlaceTitle(name),a=cleanPlaceText(fullAddress);
    // Si l'adresse exacte est celle d'une aire d'accueil, on ne remplace pas le lieu par un cimetière/parking voisin.
    if(/aire d[’']?accueil des gens du voyage/i.test(a)){
      if(/rennes/i.test(a))return 'Aire d’accueil des gens du voyage de Rennes';
      var m=a.match(/(aire d[’']?accueil des gens du voyage[^,;]*)/i);
      if(m&&m[1])return cleanPlaceTitle(m[1]);
    }
    return n;
  }
  function officialFullAddress(name,address){
    var n=cleanPlaceText(name),a=cleanPlaceText(address);
    // Adresse officielle de l'aire de Rennes quand le géocodeur renvoie seulement le nom du secteur.
  ...
  async function placeContext(lat,lon){
    ...
    try{
      ...
      if(r.ok&&j&&j.ok){
        var combinedFull=j.fullAddress||book.fullAddress||'';
        var serverName=preferredPlaceTitle(j.name||j.address||'',combinedFull);
        if((!serverName||isVagueAddress(serverName))&&book.name&&!isVagueAddress(book.name))serverName=preferredPlaceTitle(book.name,combinedFull);
        j.name=serverName||'Emplacement enregistré';
        j.address=j.name;
        j.fullAddress=officialFullAddress(j.name,combinedFull);
        return j;
      }
    }catch(_){ }
    try{
      ...
      var rawFull=(r2.ok&&j2&&j2.fullAddress?j2.fullAddress:'')||book.fullAddress||'';
      var fallback=preferredPlaceTitle(book.name||(r2.ok&&j2&&(j2.name||j2.address)?(j2.name||j2.address):''),rawFull);
      var full=officialFullAddress(fallback,rawFull);
  ...
  function saveContext(lat,lon,ctx){
    ...
    try{localStorage.setItem('return_context_v231',JSON.stringify(ctx));}catch(_){ }
    ...
    for(var i=0;i<keys.length;i++){
      try{
        ...
        if(j&&Math.abs(Number(j.lat)-Number(lat))<0.00001&&Math.abs(Number(j.lon)-Number(lon))<0.00001){
          var oldFull=j.fullAddress||localStorage.getItem('return_full_address')||'';j.name=preferredPlaceTitle(j.name||j.address||'Emplacement enregistré',oldFull);j.address=j.name;j.fullAddress=officialFullAddress(j.name,oldFull);j._loaded=true;currentCtx=j;return j;
        }
      }catch(_){ }
    }
    var near=[];try{near=JSON.parse(localStorage.getItem('return_nearby')||'[]');if(!Array.isArray(near))near=[]}catch(_){near=[]}
    var savedFull=localStorage.getItem('return_full_address')||'',savedAddress=preferredPlaceTitle(localStorage.getItem('return_address')||'Emplacement enregistré',savedFull);
    currentCtx={ok:true,name:isVagueAddress(savedAddress)?'Recherche du nom exact…':savedAddress,address:isVagueAddress(savedAddress)?'Recherche du nom exact…':savedAddress,fullAddress:officialFullAddress(savedAddress,savedFull),nearby:near,restaurants:[],fastFood:[],ratingsAvailable:false,_loaded:false,updatedAt:0};
    return currentCtx;
  }
  ...
  function shareText(lat,lon,address,fullAddress){
    return '📍 Emplacement partagé depuis Couteau Suisse\n'+(address||'Emplacement enregistré')+(fullAddress?'\n'+fullAddress:'')+'\n'+gpsUrl(lat,lon)+'\n\n📲 Télécharger Couteau Suisse :\n'+installerUrl();
  }
  ...
  function diningSummary(items,kind){
    return kind==='restaurant'?'Restaurants à moins de 10 km':'Restauration rapide (fast-food) à moins de 10 km';
  }
  async function showDiningPage(kind){
    ...
    d.innerHTML='<div style="width:min(640px,98vw);margin:0 auto;background:#0d1824;color:#fff;border:4px solid #4dc987;border-radius:24px;padding:14px;box-shadow:0 16px 50px #000"><div style="display:flex;align-items:center;gap:9px"><button id="rpDiningBack" style="min-width:76px;min-height:42px;border:0;border-radius:11px;background:#25394d;color:#fff;font:900 14px Arial">← RETOUR</button><div style="font-size:22px;font-weight:1000;flex:1">'+title+'</div></div><div style="margin-top:7px;color:#aebdcb;font-size:13px">'+(kind==='restaurant'?'Les 10 restaurants les plus proches dans un rayon de 10 km. Quand un site officiel est disponible, l’application cherche automatiquement sa page Carte / Menu / Nos plats pour afficher de vrais plats ou spécialités. Rien n’est inventé. Bouton « Réserver une table » si un numéro est disponible.':'Les 10 établissements de restauration rapide (fast-food) affichés dans un rayon de 10 km. Les grandes enseignes connues (McDonald’s, KFC, Burger King, Quick, Subway, Five Guys, O’Tacos, Domino’s, Pizza Hut, Popeyes…) passent en premier, classées par distance, puis les autres fast-foods sont classés par distance. Ils restent séparés des restaurants classiques. Quand un site officiel est disponible, l’application cherche automatiquement la carte/menu pour afficher de vrais produits ou spécialités. Rien n’est inventé.')+'</div>'+cards+'</div>';
    document.body.appendChild(d);
  ...
  function updateBubbleContext(ctx){
    ...
    var title=preferredPlaceTitle(ctx.name||ctx.address||'Emplacement enregistré',ctx.fullAddress||''),fullAddress=officialFullAddress(title,ctx.fullAddress||'');
  ...
  function showReturnBubble(lat,lon,ctx){
    ...
    d.querySelector('#rpMushroomTile').onclick=function(){location.href='/champignons.html'};
  }
