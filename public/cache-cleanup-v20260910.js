(function(){
  'use strict';
  var V='20260912-allmarkets-v149';
  var C='carplay-v5-20260912-allmarkets-v149';
  var K='carplay_cache_cleanup_version';
  var UPDATE_KEY='carplay_update_notice_seen';
  var UPDATE_ID='2026-09-12-allmarkets-v149';
  ...
  function showUpdateNotice(){
    ...
    if(document.getElementById('cpUpdateOverlayV149'))return;
    var overlay=document.createElement('div');
    overlay.id='cpUpdateOverlayV149';
    ...
    overlay.innerHTML='<div style="width:min(560px,94vw);background:#0d1826;color:#fff;border:3px solid #ff9f1a;border-radius:22px;padding:22px;box-shadow:0 18px 55px #000;text-align:left">'
      ...
      +'<div>🛒 Marchés manquants : compléments restaurés et chargés directement dans l’application.</div>'
      +'<div style="margin-top:9px">📍 Corrections intégrées : département 17, Mordelles dans le 35, Savenay et les compléments du 44, ainsi que les autres fichiers nationaux déjà vérifiés.</div>'
      +'<div style="margin-top:9px">🔄 Les mises à jour envoyées sur GitHub sont déployées vers l’application par Cloudflare.</div>'
      +'</div>'
      +'<button id="cpUpdateCloseV149" type="button" style="width:100%;margin-top:19px;padding:14px;border:0;border-radius:13px;background:#ff9f1a;color:#111;font-size:18px;font-weight:950">OK — J’AI VU</button>'
      +'</div>';
    document.body.appendChild(overlay);
    var close=document.getElementById('cpUpdateCloseV149');
