...
function boot(){clearIncompleteLegacyIdentity();var x=existing();if(complete(x)){persist(x);if(onboardingIdentity()){try{sessionStorage.setItem(HANDOFF,'1');var u=new URL(location.href);u.searchParams.delete('onboarding');history.replaceState(null,'',u.pathname+(u.searchParams.toString()?'?'+u.searchParams.toString():'')+u.hash)}catch(_){}}if(!installed()){if(!browserHandoff()){showInstall();return}}postIdentity(x).then(function(j){persistSubscription(j&&j.subscription,x);try{localStorage.removeItem('carplay_identity_sync_pending')}catch(_){}}).catch(function(){try{localStorage.setItem('carplay_identity_sync_pending','1')}catch(_){};});return}if(onboardingIdentity()){showIdentity(x);return}if(!installed()){if(browserHandoff())return;showInstall();return}showIdentity(x)}
