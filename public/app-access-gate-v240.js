...
function boot(){var x=existing();if(onboardingIdentity()){showIdentity(x);return}if(!installed()){if(browserHandoff())return;showInstall();return}if(complete(x)){persist(x);postIdentity(x).then(function(j){persistSubscription(j&&j.subscription,x)}).catch(function(){});return}showIdentity(x)}
