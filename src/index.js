...
export default {
  ...
  async fetch(request, env) {
    ...
    if (url.pathname === "/sw.js" || url.pathname === "/app-version.json" || url.pathname === "/autoradio-version.json" || url.pathname === "/voice-assist-v388.js" || url.pathname === "/voice-assist-v477.js") {
    ...
    if (type.includes("text/html") && ["/admin.html", "/admin", "/import-marches.html", "/installer.html", "/installer"].includes(url.pathname)) {
      const transformed = new HTMLRewriter().on("head", new InjectZoomOnlyFiles()).transform(response);
      const headers = new Headers(transformed.headers);
      headers.set("cache-control", "no-store, no-cache, must-revalidate");
      return new Response(transformed.body, { status: transformed.status, statusText: transformed.statusText, headers });
    }
