...
async function initializeContestTablesV301(env){
  ...
  await env.DB.prepare(`CREATE TABLE IF NOT EXISTS contest_message_receipts(message_id TEXT NOT NULL,device_id TEXT NOT NULL,seen_at INTEGER NOT NULL,PRIMARY KEY(message_id,device_id))`).run();
  await env.DB.prepare("CREATE INDEX IF NOT EXISTS contest_message_receipts_device_idx ON contest_message_receipts(device_id,seen_at DESC)").run();
...
async function contestReadMessages(request,env){const data=await body(request),sub=await contestSubscription(env,data);if(!sub)return json({ok:false,error:"ABONNEMENT_REQUIS"},403);const deviceId=String(data.deviceId||"").trim(),now=Date.now();if(validDevice(deviceId)){const rows=(await env.DB.prepare("SELECT id FROM contest_messages WHERE subscription_id=? AND read_at IS NULL ORDER BY created_at DESC LIMIT 50").bind(sub.id).all()).results||[];if(rows.length)await env.DB.batch(rows.map(r=>env.DB.prepare("INSERT OR REPLACE INTO contest_message_receipts(message_id,device_id,seen_at) VALUES(?,?,?)").bind(r.id,deviceId,now)));return json({ok:true,deviceOnly:true})}await env.DB.prepare("UPDATE contest_messages SET read_at=? WHERE subscription_id=? AND read_at IS NULL").bind(now,sub.id).run();return json({ok:true})}
